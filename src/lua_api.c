...
static int lua_set_epoch(lua_State *L)
{
    ...
    AVBufferRef *epoch_event = epoch_event_new(ctx);
    ...
    if (lua_istable(L, -1)) {
        ...
        err = epoch_event_set_source(epoch_event, obj);
        if (err < 0)
        ...
        epoch_event_set_offset(epoch_event, value);
    } else if (lua_isstring(L, -1)) {
        const char *str = lua_tostring(L, -1);

        if (!strcmp(str, "zero")) {
            epoch_event_set_offset(epoch_event, 0);
        } else if (!strcmp(str, "system")) {
            epoch_event_set_system(epoch_event);
        } else {
        ...
        epoch_event_set_external(epoch_event,
                                 epoch_external_cb,
                                 epoch_ctx_ref);
    } else {
